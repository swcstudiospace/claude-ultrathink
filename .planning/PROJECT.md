...
The TypeScript ultrathink plugin stays the source of truth for prompt uplift, Graph of Thought, and Chain of Thought. Hermes does not reimplement that engine. A Python hook calls the TypeScript pipeline, injects the uplift XML and graph, and a Hermes skill does the Notion and Linear tracking that `ultrathink-kickoff` and `ultrathink-sync` already do for Claude Code.
