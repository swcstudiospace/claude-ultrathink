...
- The `/ultrathink-off`, `-on`, `-skip`, `-status` and `-track` command files find `bin/ultrathink` when `CLAUDE_PLUGIN_ROOT` is not set, instead of failing.
