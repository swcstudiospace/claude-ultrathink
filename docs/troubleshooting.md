...
| `merge` | `review score N/5 is below 5/5`, `N open review comment(s)`, `review is for an older commit`, `no review has run` | Continue the review loop. |
...
- **Stdout is discarded.** Grok throws away the stdout of a hook that allows the prompt, so the context never reaches the model that way. The plan goes to `~/.grok/plugin-data/ultrathink/last-plan.json` (or `$GROK_PLUGIN_DATA/ultrathink/`). `apply` installs the rule `~/.grok/rules/ultrathink.md`, which tells the model to read that file, the spec it points to, and then `ultrathink-plan` and `ultrathink-kickoff`. If the model ignores the plan, check that the rule file exists, and read `last-plan.json` to confirm a plan was written for this session.
