...
describe("grokEngineLabel", () => {
	...
	test("shunt transport yields <shuntModel>@shunt", () => {
		expect(grokEngineLabel({ ...DEFAULT_GROK_CONFIG, transport: "shunt" })).toBe("grok-4.7-xhigh@shunt");
		expect(grokEngineLabel({ ...DEFAULT_GROK_CONFIG, transport: "shunt", shuntModel: "grok-4.7" })).toBe("grok-4.7@shunt");
	});
});
