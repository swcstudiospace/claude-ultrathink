	...
	if (input.statePath) {
		tail.push(
			[
				"## Ultrathink tracking",
				"",
				`Before starting work, invoke the ultrathink-kickoff skill with stateFile=${input.statePath}. It records this Task, its Graph-of-Thought Issues, and one Sub-Issue per Chain-of-Thought step of every node in Notion and Linear, resolves any blocking clarifications, and returns the final prompt to execute. Do not start coding before it returns.`,
			].join("\n"),
		);
	}
