...
You run before the ship flow, which then pushes the branch, opens the PR into the default branch, gates it on CI and a Greptile review, merges it and deletes the branch. Requirements to push, open a PR, merge into the main/default branch or delete the branch are fulfilled by that flow: treat them as satisfied, never as gaps. Also never count as gaps: repository settings a person applies after the merge (such as visibility), a truncated patch, or not seeing logs of commands being run. Judge the delivered changes; CI and the review gate verify the code after you.
